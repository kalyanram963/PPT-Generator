...
import '../components/SlidePreview.css'; // Corrected path to styles
...
const SlidePreview = ({ slide, templateStyles, templateColors, isEditing, onElementChange }) => {
  ...
  const slidePreviewRef = useRef(null); // Ref for the main slide container
  const elementRefs = useRef({}); // Refs for individual draggable elements

  useEffect(() => {
    setCurrentElements(slide.elements || []);
    elementRefs.current = {}; // Clear old refs when slide changes
  }, [slide.elements]);
  ...
  // Find the elements for easier access
  ...
  // Handle drag stop only if editing
  const handleDragStop = (elementId) => (e, ui) => {
    if (!isEditing) return; // Only allow drag stop if in editing mode
    ...
    setCurrentElements(prevElements => {
      const updatedElements = prevElements.map(el => {
        if (el.id === elementId) {
          ...
          // Calculate new position relative to the slide-preview parent
          ...
          // Also capture new width and height if resizing is enabled
  ...
  return (
    <div
      ...
      ref={slidePreviewRef} // Attach ref to the main slide container
      style={{
    ...
    >
      ...
      {isEditing ? (
        // EDITING MODE: Render Draggable elements with absolute positioning
        Array.isArray(currentElements) && currentElements.map(element => {
          ...
          const draggableProps = {
            ...
            position: { // Set initial position for draggable based on element's stored x,y
            ...
            >
              <div
                ...
                style={{
                  ...
                  cursor: 'grab',
                  resize: 'both', // Allow resizing in edit mode
                  overflow: 'auto', // Allow scroll for content overflow during edit
                  padding: '5px',
                  display: 'flex',
        ...
        // NON-EDITING MODE: Use a structured flexbox layout for proper flow
